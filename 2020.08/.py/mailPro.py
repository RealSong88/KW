...
list_txt = []
with os.scandir("e:/realsong/pycharmprojects/jump") as entries:
    for entry in entries:
        if entry.name[-3:] == "txt":
            # print(entry.name)
            list_txt.append(entry.name)

print(list_txt)

for i in list_txt:
    with open(f'e:/realsong/pycharmprojects/jump/{i}', 'r', encoding='utf8') as f:
        data = f.read()
        print(data)
